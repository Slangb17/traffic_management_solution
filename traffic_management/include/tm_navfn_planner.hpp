...
#ifndef TM_NAVFN_PLANNER__NAVFN_PLANNER_HPP_
#define TM_NAVFN_PLANNER__NAVFN_PLANNER_HPP_
...
public:
  ...
  void setCostmap(const COSTTYPE * cmap, bool isROS, bool allow_unknown);
  COSTTYPE * costarr;  /**< cost array in 2D configuration space */

  bool isNodeNotExpandable(int curIndex, int neiIndex, int traDir, double tolerance);

  std::vector<int> doDjikstra(int start, int goal);
  int * previousNode;
...
protected:
  ...
  // Traffic directions:
  std::string traffic_directions_file_path_;
  cv::Mat traffic_directions_image_;
  uint8_t * dirarr;
  bool * visited;
  int nx, ny, ns;
  int goal[2];
...
#endif  // TM_NAVFN_PLANNER__NAVFN_PLANNER_HPP_
