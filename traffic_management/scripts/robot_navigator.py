...
from rclpy.qos import QoSDurabilityPolicy, QoSHistoryPolicy, QoSReliabilityPolicy, DurabilityPolicy
...
class BasicNavigator(Node):
    def __init__(self, namespace):
        p = Parameter('use_sim_time', value=True)
        super().__init__(node_name='basic_navigator_' + namespace.split('/')[1], parameter_overrides=[p])
        ...
        # Action clients:
        self.nav_through_poses_client = ActionClient(self,NavigateThroughPoses, self.namespace + '/' + 'navigate_through_poses')
        ...
        self.compute_path_to_pose_client = ActionClient(self, ComputePathToPose, self.namespace + '/' + 'compute_path_to_pose')
        self.compute_path_through_poses_client = ActionClient(self, ComputePathThroughPoses, self.namespace + '/' + 'compute_path_through_poses')
    
        # Service clients:
        self.change_maps_srv = self.create_client(LoadMap, '/map_server/load_map')
        self.clear_costmap_global_srv = self.create_client(ClearEntireCostmap, self.namespace + '/global_costmap/clear_entirely_global_costmap')
        self.clear_costmap_local_srv = self.create_client(ClearEntireCostmap, self.namespace + '/local_costmap/clear_entirely_local_costmap')
        ...
        # Publishers:'
        self.publish_goal_on_topic = self.create_publisher(String, '/job_scheduler/goals', 10)

    def publish_goal(self, robot_name, x, y):
        msg = String()
        msg.data = robot_name + ',' + str(x) + "," + str(y)
        self.publish_goal_on_topic.publish(msg)
    ...
    def getResult(self):
        if self.status == GoalStatus.STATUS_SUCCEEDED:
            ...
            return NavigationResult.EXECUTING
        else:
            return NavigationResult.UNKNOWN

    def waitUntilNav2Active(self):
        self._waitForNodeToActivate(self.namespace + '/' + 'bt_navigator')
        self._waitForNodeToActivate(self.namespace + '/' + 'global_costmap/global_costmap')
        self._waitForNodeToActivate(self.namespace + '/' + 'planner_server')
        # PLUS MORE...
        self.info('Nav2 is ready for use!')
        return

    def isNav2Active(self):
        bt_state = self._isNodeActive(self.namespace + '/' + 'bt_navigator')
        gc_state = self._isNodeActive(self.namespace + '/' + 'global_costmap/global_costmap')
        ps_state = self._isNodeActive(self.namespace + '/' + 'planner_server')
        
        if (bt_state and gc_state and ps_state):
            self.debug('Everything is up and running on ' + self.namespace)
            return True
        return False
    ...
    def _isNodeActive(self, node_name):
        # Waits for the node within the tester namespace to become active
        self.debug('Is ' + node_name + ' active...')
        node_service = node_name + '/get_state'
        state_client = self.create_client(GetState, node_service)
        counter = 0
        while not state_client.wait_for_service(timeout_sec=0.5):
            self.debug(node_service + ' service not available...')
            if counter >= 3:
                return False
            counter += 1
