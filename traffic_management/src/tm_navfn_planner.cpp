...
#include "tm_navfn_planner.hpp"
#include "tm_navfn.hpp"
...
// Define infinity value for cost
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(
    logger_, "Configuring plugin %s of type NavfnPlanner with Traffic Directions",
    name_.c_str());
  ...
  declare_parameter_if_not_declared(
    node, name + ".traffic_directions_file", rclcpp::PARAMETER_STRING);
  node->get_parameter(name + ".traffic_directions_file", traffic_directions_file_path_);
  RCLCPP_INFO(logger_, "traffic_directions_file: %s", traffic_directions_file_path_.c_str());

  if (!traffic_directions_file_path_.empty()) {
    RCLCPP_INFO(logger_, "Loading traffic directions...");
    
    // REMEMBER TO FLIP IMAGE!!!
    traffic_directions_image_ = cv::imread(traffic_directions_file_path_, CV_8UC1);

    if (!traffic_directions_image_.empty()) {
      nx = traffic_directions_image_.cols;
      ny = traffic_directions_image_.rows;
      ns = nx * ny;
      dirarr = traffic_directions_image_.data;
      RCLCPP_INFO(logger_, "Image was loaded with success!");
    }
  }
...
void
NavfnPlanner::setNavArr(int xs, int ys)
{
...
void
NavfnPlanner::setCostmap(const COSTTYPE * cmap, bool isROS, bool allow_unknown)
{
  COSTTYPE * cm = costarr;
...
// Define node structure for Dijkstra's algorithm
struct Node {
    int index;
    double cost;
...
bool NavfnPlanner::isNodeNotExpandable(int curIndex, int neiIndex, int traDir, double tolerance) {
  if (traDir > 36) {
    return false;
  }
  int x1 = curIndex % nx;
  int y1 = -1 * ((curIndex - x1) / nx);
  int x2 = neiIndex % nx;
  int y2 = -1 * ((neiIndex - x2) / nx);
  double approachDir = atan2(-1 * (y2-y1), x2-x1);

  if (approachDir < 0.0) {
    approachDir += 2* M_PI;
  }

  double trafficDir = traDir * 10.0 * (M_PI / 180);
  double dif = approachDir - trafficDir;
  double cos_dif = cos(dif);
  // std::cout << "x1 " << x1 << " y1 " << y1 << " x2 " << x2 << " y2 " << y2 << std::endl;   
  // std::cout << "approahcDir: " << approachDir << std::endl; 
  // std::cout << "trafficDir: " << trafficDir << std::endl;
  // std::cout << "dif: " << dif << std::endl;
  // std::cout << "cos_dif: " << cos_dif << std::endl;
  // std::cout << "result:" << (cos_dif >= tolerance) << std::endl;
  if (cos_dif <= tolerance) {
      return true;
  } 
  return false;
}

std::vector<int> NavfnPlanner::doDjikstra(int start, int goal) {

  if (previousNode) {
    delete[] previousNode;
  }
  
  previousNode = new int[ns];
  memset(previousNode, 0, ns * sizeof(int));
  ...
  visited = new bool[ns]; 
  memset(visited, 0, ns * sizeof(bool));

  // Create priority queue to hold nodes with lowest cost
  std::priority_queue<Node, std::vector<Node>, std::greater<Node>> pq;

  // Create vector to hold distance/cost to each node from start
  std::vector<double> distance(ns, INF);

  // Mark the start node as visited and add to priority queue
  distance[start] = 0.0;
  pq.push({ start, 0.0 });
  ...
  while (!pq.empty()) {
    ...
    //std::cout << "index: " << curr.index << std::endl;

    int x = curr.index % nx;
    int y = (curr.index - x) / nx;
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        ...
        if (x_ne < 0 || x_ne >= nx || y_ne < 0 || y_ne >= ny) {
          continue;
        }

        int neighbor = x_ne + nx * y_ne;
        
        // Check if neighbor is not a lethal obstacle or is expandable:
        if (costarr[neighbor] >= COST_OBS) {
          continue;
        }


        if (isNodeNotExpandable(curr.index, neighbor, dirarr[neighbor], -0.1)) {
          continue;
        }

        float cost = costarr[neighbor] * 1.0;
        float stepCost = 1.0;
        // Check if neighbor is a diagonal node
        if (abs(i) == 1 && abs(j) == 1) {
          stepCost = std::sqrt(2);
        }

        // If the new distance/cost to this neighbor is lower than current
        if (distance[curr.index] + cost + stepCost < distance[neighbor]) {
          // Update the distance/cost and add to priority queue
          distance[neighbor] = distance[curr.index] + cost + stepCost;
          previousNode[neighbor] = curr.index;
        }
  ...
  // Create vector to hold shortest path from start to goal
  std::vector<int> path;

  // If we found a path to the goal, backtrack to get the shortest path
  ...
  //   int x = index % nx;
  //   int y = (index - x) / nx;
  //   std::cout << "Path: " << "index: " << index << " x" << x << " y " << y << " dir " << (int)dirarr[index] << std::endl; 
  // }
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  setCostmap(costmap_->getCharMap(), true, allow_unknown_);
  ...
  std::vector<int> path = doDjikstra(map_start[0] + map_start[1] * nx, map_goal[0] + map_goal[1] * nx);
  ...
  if (!path.empty()) {
    for (std::vector<int>::size_type i = 0; i < path.size(); i++) {
      ...
      if (i == path.size()-1) {
        ...
        auto approach_pose = plan.poses[path.size() - 2].pose.position;
        double dx = last_pose.x - approach_pose.x;
        double dy = last_pose.y - approach_pose.y;
      ...
      "Failed to create a create a path since no path was found by Djikstra");
    }
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  int path_len = planner_->calcPath(max_cycles, NULL, dirarr);
  if (path_len == 0) {
    return false;
  }
...
PLUGINLIB_EXPORT_CLASS(tm_navfn_planner::NavfnPlanner, nav2_core::GlobalPlanner)
